...
func GetSubMonth(beginData, endData string) int {
	beginArray := strings.Split(beginData, "-")
	endArray := strings.Split(endData, "-")

	beginY, _ := strconv.Atoi(beginArray[0])
	endY, _ := strconv.Atoi(endArray[0])

	beginM, _ := strconv.Atoi(beginArray[1])
	endM, _ := strconv.Atoi(endArray[1])

	return (endY-beginY)*12 + endM - beginM
}
